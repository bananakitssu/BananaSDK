...
public:
    ...
    CircleProgress(float x, float y, float w, float h);
    ...
    void SetCycleTime(float seconds);     // indeterminate arc cycle, default 1.6f
    void SetMorphTime(float seconds);     // dot->ring grow duration, default 0.4f
...
private:
    float m_X=0, m_Y=0, m_W=80.0f, m_H=80.0f;
    float m_Stroke    = 8.0f;
    float m_Speed     = 8.0f;
    float m_CycleTime = 1.6f;
    float m_MorphTime = 0.4f;
    ...
    float m_MorphElapsed = 0.0f;
    float m_RotOffset = 0.0f;
    float m_ArcPhase  = 0.0f;
